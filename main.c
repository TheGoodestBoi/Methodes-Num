...
//----------------------------------------------------------------------------------------- DECLARATION DES VARIABLES
	...
	int i,j;
	int n;
	printf("Entrer le nombres de divisions de la corde : ");
	scanf("%d", &n);
//double L[n][n], Lt[n][n];
//double I[n][n]; for (i = 0 ; i < n ; ++i) {for (j = 0 ; j < n ; ++j) {if (i == j) {I[i][j] = 1;} else {I[i][j] = 0;} }}
	double vectP[n];
	double vectP2[n];
	double lamb = 0;
	double x0[n];
...
//--------------------------------------------------------------------------------------- REMPLISSAGE DE LA MATRICE M
	...
	for(i = 0 ; i < n ; ++i) {
		for(j = 0 ; j < n ; ++j) {
			M[i][j] *= ((sigma * pow(n, 2))/(rho * pow(Long, 2))) ;
			printf("%.0f ",M[i][j]);
		}
		printf("\n");
	}
...
puls = sqrt(puissance_inverse(n, eps, M, I, vectP, x0);
